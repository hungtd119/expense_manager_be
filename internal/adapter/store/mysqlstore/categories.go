...
func (s *MySQLStore) CreateCategory(category Category) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if err := insertMySQLCategory(db, category); err != nil {
...
func (s *MySQLStore) UpdateCategory(category Category) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *MySQLStore) DeleteCategory(userID string, id string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func insertMySQLCategory(db execer, category Category) error {
	_, err := db.Exec(`
