...
func (s *SQLiteStore) CreateWallet(wallet Wallet) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if err := insertSQLiteWallet(db, wallet); err != nil {
...
func (s *SQLiteStore) UpdateWallet(wallet Wallet) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *SQLiteStore) DeleteWallet(userID string, id string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
