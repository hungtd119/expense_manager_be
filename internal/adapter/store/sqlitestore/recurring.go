...
func (s *SQLiteStore) ListRecurring(userID string) ([]RecurringTransaction, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *SQLiteStore) CreateRecurring(recurring RecurringTransaction) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	return insertSQLiteRecurringTransaction(db, recurring)
}

func (s *SQLiteStore) UpdateRecurring(recurring RecurringTransaction) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *SQLiteStore) SoftDeleteRecurring(userID string, id string, deletedAt string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *SQLiteStore) ProcessDueRecurring(userID string, untilAt string) (RecurringResult, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	for _, recurring := range recurringItems {
		...
		for runAt != "" && runAt <= untilAt && guard < 36 {
			...
			if !exists {
				...
				if err := insertSQLiteTransaction(tx, generated); err != nil {
...
func insertSQLiteRecurringTransaction(db execer, recurring RecurringTransaction) error {
