...
func (s *SQLiteStore) ListTransactionsForMonth(userID string, bounds MonthBounds) ([]Transaction, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *SQLiteStore) CreateTransaction(tx Transaction) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	return insertSQLiteTransaction(db, tx)
}

func (s *SQLiteStore) UpdateTransaction(tx Transaction) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *SQLiteStore) SoftDeleteTransaction(userID string, id string, deletedAt string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func insertSQLiteTransaction(db execer, tx Transaction) error {
