...
func (s *SQLiteStore) ListBudgetsForMonth(userID string, bounds MonthBounds) ([]Budget, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *SQLiteStore) CreateBudget(budget Budget) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if err := insertSQLiteBudget(db, budget); err != nil {
...
func (s *SQLiteStore) UpdateBudget(budget Budget) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *SQLiteStore) SoftDeleteBudget(userID string, id string, deletedAt string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func insertSQLiteBudget(db execer, budget Budget) error {
